...
def train(modelConfig):
    ...
    writer = SummaryWriter(log_dir='./logs/' + time_stamp)  # save logs to the path

    # get data
    spectral_sub0 = data_input_process(modelConfig)
    target = spectral_sub0[1]
    bkg = spectral_sub0[2]

    target_CDF = (target)
    bkg_CDF = (bkg)
    indices = torch.randperm(target_CDF.shape[0])
    ...
    Optimizer_Trans_Encoder = optim.Adam(Transformer_Encoder.parameters(), lr=0.0005, weight_decay=1e-4)
    Optimizer_CCN = optim.Adam(CCN.parameters(), lr=0.001)

    print("=====> Start Training")
    target_CDF = target_CDF.view(target_CDF.shape[0], modelConfig['n_clip'], d_model)
    bkg_CDF = bkg_CDF.view(bkg_CDF.shape[0], modelConfig['n_clip'], d_model)
    ...
    for epoch in range(1):
        ...
        # for positive in data_loader:
            ...
            bkg_encode = Transformer_Encoder(bkg_CDF)
            # ------------------------------------
            # ------------------------------------
            ...
            # ------------------------------------
            # ------------------------------------
            BCE_loss_i = (BCE_loss_fun(positive_pred, P_label) + BCE_loss_fun(hard_neg_pred, torch.zeros_like(hard_neg_pred))) / 2
            (BCE_loss_i).backward()

            Optimizer_Trans_Encoder.step()
            Optimizer_CCN.step()

            Triplet_loss = Triplet_loss_i.detach() + Triplet_loss
            BCE_loss = BCE_loss_i.detach() + BCE_loss
            # ------------------------------------
            # ------------------------------------
        Triplet_loss = Triplet_loss / target_CDF.shape[0]
        BCE_loss = BCE_loss / target_CDF.shape[0]
    ...
    CCN = CCN_Fin_Tuning(target_CDF, bkg_CDF, Transformer_Encoder, CCN, modelConfig, BL, device, writer)
    ...
    return 0
def CCN_Fin_Tuning(target_CDF, bkg_CDF, Transformer_Encoder, CCN, modelConfig, BL, device, writer):
    Optimizer_CCN = optim.Adam(CCN.parameters(), lr=0.001)
    d_model = modelConfig['sub_band'] // modelConfig['n_clip']
    P_label = torch.ones(1, 1, 1).to(device)
    BCE_loss_fun = nn.BCELoss()
    ...
    # ------------------------------------
    indices = torch.randperm(target_CDF.shape[0])
    target_CDF_shuffled = target_CDF[indices]
    a = target_CDF.shape[0] // 2
    target_CDF_shuffled = target_CDF_shuffled[0:a, :, :]

    for epoch in range(modelConfig['epoch']):
        BCE_loss = torch.tensor(0).to(device)
        for i in range(target_CDF_shuffled.shape[0]):
            CCN.zero_grad()
            positive = target_CDF_shuffled[i, :, :]
            positive = positive.unsqueeze(0)
...
def SemiHard_Neg_Minning(positive_encode, bkg_encode, anchor_encode):
    ...
    upper_bound = Distance_positive2anchor + 0.5
