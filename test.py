...
def test(modelConfig):
    ...
    # load data
    hs_CDF, gt = load_test_data(modelConfig)
    BL = RDP_process(modelConfig)
    hs_CDF = hs_CDF.to(device)
    BL = BL.to(device)
    ...
    Cross_Pred = CombineCov_Net(modelConfig['n_clip'], modelConfig['sub_band']).to(device)
    Cross_Pred.load_state_dict(torch.load(path_model_pth + 'CCN.pth', map_location=device))
    ...
    hs_CDF = hs_CDF.view(hs_CDF.shape[0], modelConfig['n_clip'], d_model)

    start_time = time.time()
    hs_encode = Transformer_Encoder(hs_CDF)
    RDP_Sample_encode = Transformer_Encoder(RDP_sample_CDF)
    Pred = Cross_Pred(RDP_Sample_encode, hs_encode)
    end_time = time.time()
...
if __name__ == '__main__':
    modelConfig = {
        ...
        'epoch': 1000,  #
        'alpha': 0.2,  # margin
        'n_heads': 1,  # number of multi-head attention
        'd_ff': 512,  # Dimension of feedforward
        'n_layers': 2,  # number of transformer layer
        'n_clip': 3,  # number of clips
        'sub_band': 6,  # number of band in CS
